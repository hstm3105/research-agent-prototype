...
import { BookOpenText, ChevronRight, CircleAlert, FileSearch, Loader2, LogOut, PanelLeft, Plus, RefreshCw, Search, Settings2 } from "lucide-react";
...
export function getResearchDepthLabel(depth: ResearchSessionNavItem["researchDepth"]) {
  return depth === "deep" ? "Deep" : depth === "quick" ? "Quick" : "Standard";
}

export function filterResearchSessions(sessions: ResearchSessionNavItem[], query: string) {
  const term = query.trim().toLowerCase();
  if (!term) return sessions;
  return sessions.filter(session => `${session.title} ${session.status} ${session.researchDepth}`.toLowerCase().includes(term));
}
...
function ResearchSidebar({ children, sessions = [], selectedSessionId, onNewResearch, onSelectSession, onSettings, isSessionsLoading, sessionsError, onRetrySessions, onResize }: SidebarProps) {
  ...
  const filteredSessions = useMemo(() => filterResearchSessions(sessions, historyQuery), [historyQuery, sessions]);
  ...
  return <>
    <div className="relative">
      <Sidebar collapsible="icon" className="border-r-0" disableTransition={isResizing}>
        ...
        <SidebarContent className="gap-0 px-2 pt-4"><div className="mb-2 flex items-center justify-between px-2 group-data-[collapsible=icon]:hidden"><span className="font-mono-ui text-[10px] uppercase tracking-[0.16em] text-muted-foreground">Recent research</span><BookOpenText className="h-3.5 w-3.5 text-muted-foreground" /></div><div className="mb-3 px-1 group-data-[collapsible=icon]:hidden"><label className="sr-only" htmlFor="session-history-search">Search research history</label><div className="relative"><Search className="pointer-events-none absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-muted-foreground" /><Input id="session-history-search" value={historyQuery} onChange={event => setHistoryQuery(event.target.value)} placeholder="Search history" className="h-9 rounded-lg border-border bg-muted/45 pl-8 text-xs" /></div></div><SidebarMenu className="gap-1 px-0">
          {isSessionsLoading ? Array.from({ length: 3 }).map((_, index) => <div key={index} className="flex h-12 items-center gap-3 rounded-xl px-3 group-data-[collapsible=icon]:hidden"><Loader2 className="h-3.5 w-3.5 animate-spin text-primary/55" /><span className="h-2.5 flex-1 rounded-full bg-muted" /></div>) : sessionsError ? <div className="rounded-xl border border-rose-200 bg-rose-50 px-3 py-3 text-xs leading-5 text-rose-800 group-data-[collapsible=icon]:hidden"><div className="flex gap-2"><CircleAlert className="mt-0.5 h-3.5 w-3.5 shrink-0" />History could not be loaded.</div><button onClick={onRetrySessions} className="mt-2 inline-flex items-center gap-1 font-medium text-rose-900 hover:underline"><RefreshCw className="h-3 w-3" /> Retry</button></div> : sessions.length ? filteredSessions.length ? filteredSessions.map(session => <SidebarMenuItem key={session.id}><SidebarMenuButton isActive={selectedSessionId === session.id} onClick={() => onSelectSession?.(session.id)} tooltip={session.title} className="h-auto min-h-11 items-start rounded-xl py-2.5"><FileSearch className={`mt-0.5 h-4 w-4 shrink-0 ${selectedSessionId === session.id ? "text-primary" : "text-muted-foreground"}`} /><span className="flex min-w-0 flex-1 flex-col gap-1 group-data-[collapsible=icon]:hidden"><span className="truncate text-sm font-medium leading-none">{session.title}</span><span className="flex items-center gap-1.5 font-mono-ui text-[9px] uppercase tracking-wide text-muted-foreground"><span className="rounded bg-primary/10 px-1 py-0.5 text-primary">{getResearchDepthLabel(session.researchDepth)}</span><span className={`h-1.5 w-1.5 rounded-full ${session.status === "complete" ? "bg-emerald-500" : session.status === "failed" ? "bg-rose-500" : "bg-amber-500"}`} />{session.status.replace(/_/g, " ")}</span></span></SidebarMenuButton></SidebarMenuItem>) : <div className="rounded-xl border border-dashed border-border px-3 py-4 text-xs leading-relaxed text-muted-foreground group-data-[collapsible=icon]:hidden">No saved research matches “{historyQuery}”.</div> : <div className="rounded-xl border border-dashed border-border px-3 py-4 text-xs leading-relaxed text-muted-foreground group-data-[collapsible=icon]:hidden">Your research sessions will collect here.</div>}
